...
def read_idx(name):
    ...
    with open('res/'+name, 'rb') as fp:
        while 1:
            ...
            cur = text
    return index_table
def main(name,idx_suffix,grp_suffix):
    os.makedirs("./img_export/"+name, exist_ok=True)
    
    null_color = [0,0,0,0]
    colors = col()
    index_table = read_idx(name+idx_suffix)
    
    with open('res/'+name+grp_suffix, 'rb') as fp:
        file_name = 0
            ...
            if index_data > 0:
                ...
                while 1:
                    ...
                    if row_idx == h:
                        break
                if w>1 and h>1:
                    savepng("img_export/"+name+"/"+str(file_name),data)
            file_name += 1
